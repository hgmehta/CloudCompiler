...
def getTotalFileNo():
    ...
    try:
        ...
        if conn.is_connected():
            cursor = conn.cursor();
            cursor.execute("SELECT DISTINCT _language, COUNT(fileType), icon FROM repository INNER JOIN languages ON repository.fileType = languages.extension GROUP BY fileType")
